    ...
    <div style={{
      ...
      <div className="fade-in" style={{
        ...
        <div style={{ padding: '1rem', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2>Seu Carrinho</h2>
          <button onClick={onClose} style={{ background: 'transparent', color: 'var(--text-primary)', padding: 0 }}><FaTimes size={24} /></button>
        </div>
          ...
          {cartItems.length === 0 ? (
            ...
            cartItems.map((item) => (
              <div key={`${item.id}-${item.type}`} style={{
                ...
                <img src={item.image} alt={item.title} style={{ width: 60, height: 60, objectFit: 'contain', borderRadius: '4px', backgroundColor: '#fff' }} />
                <div style={{ flex: 1 }}>
                  <h4 style={{ margin: '0 0 0.5rem 0', fontSize: '0.9rem' }}>{item.title}</h4>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ color: 'var(--accent-secondary)' }}>€ {item.price.toFixed(2)}</span>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <button onClick={() => updateQuantity(item.id, item.type, -1)} style={{ padding: '0.2rem 0.5rem', fontSize: '0.8rem' }}><FaMinus /></button>
                      <span>{item.quantity}</span>
                      <button onClick={() => updateQuantity(item.id, item.type, 1)} style={{ padding: '0.2rem 0.5rem', fontSize: '0.8rem' }}><FaPlus /></button>
                    </div>
                  </div>
                </div>
